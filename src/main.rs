...
use crate::automations::api::{
    ...
    list_automations, preview_expression, run_node, update_automation,
};
...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/automations/{id}/preview", post(preview_expression))
        .route("/api/automations/{id}/run-node", post(run_node))
        .route(
            "/api/automations/{id}",
